...
const CreateStore = () => {
  ...
  useEffect(() => {
    const fetchHubs = async () => {
      try {
        ...
        // Allow assigning store to any valid hub (Active, Approved or newly created Pending)
        setAvailableHubs(data.filter(h => ['Active', 'Approved', 'Pending'].includes(h.status)));
      } catch (err) {
  ...
  const validate = () => {
    ...
    if (!formData.hubId) newErrors.hubId = 'Please select a Hub to assign this store to';
  ...
  const handleSubmit = async (e) => {
    ...
    const isAnyEmpty = !formData.name.trim() || !formData.ownerName.trim() || !formData.mobile.trim() || 
                       !formData.email.trim() || !formData.address.trim() || !formData.hubId;
    ...
    try {
      ...
      await superAdminService.createStore(formData);
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              <select 
                ...
                onChange={handleChange}
                className={`${inputClasses('hubId')} appearance-none`}
              >
                <option value="">-- Select an Active Hub --</option>
                {availableHubs.map(hub => (
                  <option key={hub.id} value={hub.id}>
                    {hub.name} {hub.status !== 'Active' ? `(${hub.status})` : ''}
                  </option>
                ))}
              </select>
            </div>
            {errors.hubId && <p className="text-xs font-bold text-rose-500 mt-1">{errors.hubId}</p>}
          </div>
