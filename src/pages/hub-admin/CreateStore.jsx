...
import { validateKenyaMobile } from '@/services/super-admin/superAdminService';
...
const CreateStore = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    address: '',
    longitude: '',
    hubId: userData?.hubId || userData?.entityId || ''
  });
  ...
  useEffect(() => {
    if (userData?.hubId || userData?.entityId) {
      setFormData(prev => ({ ...prev, hubId: userData.hubId || userData.entityId }));
    }
  }, [userData]);
  ...
  const validate = () => {
    ...
    if (!formData.hubId) newErrors.hubId = 'Internal Error: Hub ID missing';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const exists = await hubAdminService.checkEmailExists(formData.email);
      if (exists) {
      ...
      await hubAdminService.createStore(formData);
      setPopup({ message: 'Store created successfully as Pending', type: 'success' });
      
      setTimeout(() => navigate('/hub-dashboard/stores'), 1500);
    } catch (error) {
  ...
  return (
    <div className="max-w-2xl mx-auto py-8 animate-fade-in relative">
      ...
      <div className="mb-8">
        <h2 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-800'}`}>Add New Store</h2>
        <p className={`text-sm font-medium mt-1 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>Register a new retail store under your hub.</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-4 mt-2">
            <button
              ...
              className={`flex items-center gap-2 text-sm font-bold transition-colors ${
                isDark ? 'text-brand hover:text-brand-hover' : 'text-brand hover:text-brand-hover'
              }`}
            >
              <MapIcon size={16} />
              Select on Map
          ...
          <div className="pt-6 border-t border-slate-200 dark:border-slate-700 flex items-center justify-end gap-4">
            <button 
              type="button"
              onClick={() => navigate(-1)}
              className={`px-6 py-2.5 rounded-xl font-bold text-sm transition-all ${isDark ? 'text-slate-400 hover:bg-slate-800' : 'text-slate-500 hover:bg-slate-100'}`}
            >
              Cancel
            </button>
            <button 
              ...
              className="px-10 py-2.5 rounded-xl font-bold text-sm bg-brand text-white hover:bg-brand-hover shadow-lg shadow-brand/20 transition-all flex items-center gap-2"
            >
