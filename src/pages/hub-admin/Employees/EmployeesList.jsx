...
const EmployeesList = () => {
  ...
  const [viewMode, setViewMode] = useState(localStorage.getItem('hubEmployeesViewMode') || 'list');

  const handleViewModeChange = (mode) => {
    setViewMode(mode);
    localStorage.setItem('hubEmployeesViewMode', mode);
  };
    ...
    <div className="space-y-8 pb-8 animate-fade-in">
      ...
      >
        ...
        {viewMode === 'list' ? (
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 bg-slate-50/50 dark:bg-slate-800/10">
            {paginatedEmployees.length > 0 ? paginatedEmployees.map((emp) => (
              <div key={emp.id} className={`flex flex-col p-6 rounded-2xl border transition-all duration-300 hover:-translate-y-1 hover:shadow-xl ${isDark ? 'bg-[#2c3136] border-slate-700' : 'bg-white border-slate-200'}`}>
                <div className="flex justify-between items-start mb-4">
                  <div className="w-14 h-14 rounded-full flex items-center justify-center font-bold text-xl text-white bg-gradient-to-br from-indigo-500 to-purple-500 shadow-md">
                    {emp.fullName.charAt(0).toUpperCase()}
                  </div>
                  <span className={`px-2.5 py-1 rounded-md text-[10px] font-black uppercase tracking-wider ${isDark ? 'bg-slate-800 text-slate-300' : 'bg-slate-100 text-slate-600'}`}>
                    {emp.employeeId}
                  </span>
                </div>
                
                <div className="mb-4">
                  <h3 className={`text-lg font-bold ${isDark ? 'text-white' : 'text-slate-800'}`}>{emp.fullName}</h3>
                  <p className={`text-sm font-medium ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{emp.designation}</p>
                </div>

                <div className="space-y-2.5 mb-6 flex-1">
                  <div className={`flex items-center gap-2 text-xs font-semibold ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
                    <Briefcase size={14} className="text-brand opacity-80" />
                    {emp.department} • {emp.employeeType}
                  </div>
                  <div className={`flex items-center gap-2 text-xs font-semibold ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
                    <Phone size={14} className="text-emerald-500 opacity-80" />
                    {emp.phone}
                  </div>
                  <div className={`flex items-center gap-2 text-xs font-semibold ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
                    <Mail size={14} className="text-amber-500 opacity-80" />
                ...
                <div className="pt-4 border-t border-slate-100 dark:border-slate-700/50">
                  <button 
                    onClick={() => navigate(`/hub-dashboard/employees/${emp.id}`)}
                    className="w-full flex items-center justify-center gap-2 py-2 rounded-lg bg-brand/10 text-brand hover:bg-brand hover:text-white transition-colors text-xs font-bold"
                  >
              ...
              <div className="col-span-full py-10 text-center text-slate-500 text-sm font-medium">
                No employees found matching your search.
              </div>
            )}
          </div>
