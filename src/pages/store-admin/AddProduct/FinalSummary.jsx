...
const DetailsCell = ({ details }) => {
  ...
  if (details.length <= 40) return <span>{details}</span>;
  
  return (
    <div className="details-cell">
      <span>{isExpanded ? details : `${details.substring(0, 40)}...`}</span>
      <button 
        onClick={() => setIsExpanded(!isExpanded)}
        style={{ color: '#1d5ba0', background: 'none', border: 'none', padding: 0, marginLeft: '4px', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold', textDecoration: 'underline' }}
      >
...
const FinalSummary = () => {
  ...
  const handleBack = () => navigate('/store-dashboard/add-product', { state: { products } });

  const { userData } = useAuth();
  const handleConsoleSubmit = async (e) => {
    if (e) e.preventDefault();
    if (!userData?.entityId) {
      alert("No entity ID found for this user.");
      return;
    }
    setLoading(true);
    try {
      const storeId = userData.entityId;
      ...
      const promises = products.map((p) => {
        const productData = {
          ...
          category: p.category,
          brand: p.brand || '',
          quantity: Number(p.itemsCount) || 1,
          image: p.imageUrl || '',
          weight: p.weight || "N/A",
          details: p.details || '',
          status: 'Active'
        };
        return storeAdminService.addProduct(storeId, productData);
      });

      await Promise.all(promises);
      setIsSubmitted(true);
      setTimeout(() => navigate('/store-dashboard/products'), 2000);
    } catch (error) {
      console.error('Error saving products:', error);
      alert('Failed to save products to database. Please try again.');
    } finally {
      setLoading(false);
    }
  };
                  ...
                  {products.map((p, i) => {
                    ...
                    return (
                      <tr key={p.id}>
                        ...
                        <td className="td-price">KES {Number(p.sellingPrice).toFixed(2)}</td>
                        <td className="td-mrp">
                          <span className={discount > 0 ? 'strikethrough' : ''}>KES {Number(p.mrp).toFixed(2)}</span>
                        </td>
                        <td>
                          {discount > 0
