...
const SidebarForm = ({ onAddProduct, productToEdit, clearEditProduct }) => {
  ...
  const customBrandNames = []; // Mocked for store-admin
  const defaultBrands = formData.productName ? productBrands[formData.productName] || [] : [];
  const availableBrands   = Array.from(new Set([...defaultBrands, ...customBrandNames]));
